...
export const API_ROUTES = {
  GET: [${routes.getRoutes.map((route) => `'${route.replace('api/v2', '')}'`).join(',\n    ')}] as const,
  POST: [${routes.postRoutes.map((route) => `'${route.replace('api/v2', '')}'`).join(',\n    ')}] as const,
  PUT: [${routes.putRoutes.map((route) => `'${route.replace('api/v2', '')}'`).join(',\n    ')}] as const,
  DELETE: [${routes.deleteRoutes.map((route) => `'${route.replace('api/v2', '')}'`).join(',\n    ')}] as const,
};
